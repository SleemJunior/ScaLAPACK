...
//
//  A.init([nD](int i, int j){ return nD*i + j; });
//  B.init([](int,int){ return 2.0; });
//
//  dist_mat_t C = matmul(A, B);
//
//  C.print();
//}
...
int main(int argc, char** argv) {
  ...
  int num_mult = 7;
  int num_input_mat = num_mult + 1;
  vector<dist_mat_t> mats;
  mats.reserve(num_input_mat + num_mult);

  for(int i = 0; i != num_input_mat; ++i) {
    mats.emplace_back(setting, dims, blocks);
    mats.back().init(f_random);
  }
  ...
  double elapsed_time = -1.0;
  {
    ...
    for(int i = 0; i != num_mult; ++i) {
      ...
      // Multiply and put the output into the vector of matrices
      mats.push_back(std::move(matmul(lhs, rhs)));
    }
  }
  if(setting.rank == 0) {
    std::cout << 
      ...
      "Number of multiplies " << num_mult << std::endl << 
